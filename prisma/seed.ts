...
async function main() {
  ...
  try {
    ...
    await prisma.reservation.deleteMany({}).catch((err) => {
      console.warn('⚠️ No Reservation model or no reservations to delete:', err.message);
    });
    await prisma.listing.deleteMany({});
    console.log('✅ Existing data deleted.');

    // 2. Seed listings with upsert
    ...
    for (const listing of listings) {
      ...
      console.log(`✅ Seeded listing: ${listing.title} (ID: ${id})`);
    }
    ...
    if (sampleListing) {
      console.log('📋 Sample listing:', {
        ...
        maxGuests: sampleListing.maxGuests
      });
    }
